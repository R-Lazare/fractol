...
/*   Updated: 2023/07/13 21:19:59 by rluiz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mandelbrot(t_data img)
{
	...
	while (x <= img.width)
	{
		...
		while (y <= img.height)
		{
			...
			if (m == max_iter)
				my_mlx_pixel_put(&img, (int)x, (int)y, create_trgb(0, 20, 0,
						3));
			else
				my_mlx_pixel_put(&img, (int)x, (int)y, create_trgb(0, (int)(m
							* cos(m) * 255 / max_iter + 50 * (m < max_iter)), 10
						* (m < max_iter), ((m * 255 / max_iter) * (m < max_iter)
							+ 20 * (m < max_iter)) % 256));
			y++;
		}
		x++;
